...
resource "aws_api_gateway_rest_api" "lawly-serverless-api" {
...
resource "aws_api_gateway_deployment" "lawly-serverless-api" {
  rest_api_id = aws_api_gateway_rest_api.lawly-serverless-api.id

  lifecycle {
    create_before_destroy = true
  }
  triggers = {
    redeployment = sha1(jsonencode(aws_api_gateway_rest_api.lawly-serverless-api.body))
  }
  
}
// the api gateway stage
resource "aws_api_gateway_stage" "lawly-serverless-api" {
  deployment_id = aws_api_gateway_deployment.lawly-serverless-api.id
  rest_api_id   = aws_api_gateway_rest_api.lawly-serverless-api.id
...
resource "aws_lambda_permission" "lambda_permission_auth" {
  ...
  //source_arn = "${aws_api_gateway_rest_api.lawly-serverless-api.execution_arn}/*/*/*"
}
...
/*resource "aws_api_gateway_domain_name" "api-gateway-domain-name" {
  domain_name              = "api-stage.lawly.io"
...
resource "aws_api_gateway_base_path_mapping" "api-gateway-base-path-mapping" {
  ...
  api_id = aws_api_gateway_rest_api.lawly-serverless-api.id
  stage_name  = aws_api_gateway_stage.lawly-serverless-api.stage_name
}*/
